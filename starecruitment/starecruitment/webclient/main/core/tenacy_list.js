require(['../../js/require.config'], function() {
  require([
    ...
    layui.use(['layer', 'form', 'element', 'laypage', 'table'], function() {
      ...
      localStorage.URL = 'tenacy_list.htm';
      page = 1;
      num = 20;
      status = -1;
      ...
      //监听修改
      table.on('edit(t_list)', function(obj) {
        var value = obj.value; //得到修改后的值
        var data = obj.data; //得到所在行所有键值
        var field = obj.field; //得到字段
        updateMark(data.ID, value);
        //layer.msg('[ID: ' + data.ID + '] ' + field + ' 字段更改为：' + value);
      });

      //头工具栏事件
      table.on('toolbar(t_list)', function(obj) {
        ...
        switch (obj.event) {
          ...
          case 'del': //删除
            var data = checkStatus.data;
            if (data.length > 0) {
              layer.confirm('确定要删除一共' + data.length + '个租户', function(index) {
                deleteTenacy(data, index);
              });
            } else {
              layer.msg("没有选中任何行");
            }
      ...
      function updateStatus(data, status) {
        ...
        for (var i = 0; i < data.length; i++) {
          CACHEPOOL.insertkey("keyword", "Tenacy");
          CACHEPOOL.insertkey("command", "updateTenacyStatus");
      ...
      function updateMark(ID, Mark) {
        var index=layer.load(1);
        CACHEPOOL.insertkey("keyword", "Tenacy");
        CACHEPOOL.insertkey("command", "updateTenacyMark");
        CACHEPOOL.insertkey("Mark", Mark);
      ...
      //根据id硬删除租户信息
      function deleteTenacy(data, index) {
        for (var i = 0; i < data.length; i++) {
          CACHEPOOL.insertkey("keyword", "Tenacy");
          CACHEPOOL.insertkey("command", "deleteTenacy");
          CACHEPOOL.insertkey("ID", data[i].ID);
          CACHEPOOL.nocachecommand();
        }
        CACHEPOOL.calculate(function() {
      ...
      function setPage() {
        var index = layer.load(1);
        CACHEPOOL.insertkey("keyword", "Tenacy");
        CACHEPOOL.insertkey("command", "getTenacyList");
